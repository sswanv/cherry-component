...
func (f *SourceFile) Init(_ IDataConfig) {
	...
	if err != nil {
		clog.Fatalf("Unmarshal fileConfig fail. err = %v", err)
		return
	}
	...
	if err != nil {
		clog.Fatalf("AddFilterHook extName fail. err = %v", err)
		return
	}
	...
	if err != nil {
		clog.Fatalf("[name = %s] join path fail. err = %v.", f.Name(), err)
		return
	}

	err = f.watcher.Add(f.monitorPath)
	if err != nil {
		clog.Fatalf("New watcher error. path=%s, err = %v", f.monitorPath, err)
		return
	}
